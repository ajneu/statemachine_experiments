 ...
 protected:
  void onEntry(QEvent *event) {
    if (timerRunning) {
      ...
      case QEvent::Type(DEventTimeout):
        {
...
class StateMachine : public QStateMachine {
 ...
 StateMachine(const std::string &name_, QObject * parent = nullptr)
   : QStateMachine{parent},            name{name_}, timersRunning{true},
     ...
     /*
     , transX_toPing{&statePong},
       transX_toPong{&statePing},
       transI{&stateTop},
       transO{&stateTop},
       transTimout_toPing{always_true<UserDGEventTransition<UserDEventTimeout>::Data>, &statePong}, // see member declaration below for commentary
       transTimout_toPong{&statePing}
     */
 {
 ...
 State statePing;
 State statePong;
 UserEventTransition<UserEventX> transX_toPing; // statePong --- transX_toPing ---> statePing
 UserEventTransition<UserEventX> transX_toPong; // statePing --- transX_toPong ---> statePong
 UserEventTransition<UserEventI> transI;        // stateTop  --- transI ----------> statePing
 UserEventTransition<UserEventO> transO;        // stateTop  --- transO ----------> statePong
 UserEventTransition<UserEventT> transT;        /* stateTop  --- transT ----------|
 ...
 */
 ...
 UserEventTransition<UserDEventTimeout>     transTimout_toPong; // statePing --- transTimout_toPong ---> statePong
};
